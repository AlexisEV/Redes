...
void pedirYEnviarP(int sock, char simbolo)
{
    int pos;
    do {
        std::cout << "Tu turno (" << simbolo << "). Posición (1‑9): ";
        std::cin  >> pos;
    } while (pos < 1 || pos > 9);
...
int main(void)
{
  ...
  while (!salir)
  {
    ...
    if (!enPartida)
    {
      std::cout << "6. Unirse a tic tac toe"<< std::endl;
    }
    ...
    else if (opcion == 6)
    {
      ...
      write(socketCliente, buffer, 6);
      enPartida = true;
      // El cliente queda bloqueado mientras juega (no peude realizar otras acciones))
      while (enPartida)
        ;
    }
    else
